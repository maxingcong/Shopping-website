...
function ForeachFiles($dir="", $pre_dir=""){
    if(empty($dir)){
        $dir = "../".WEBDIRNAME;
    }
	$dir = str_replace("\\","/",$dir);
	...
	while(($file = readdir($cur_dir))!==false){//readdir()返回打开目录句柄中的一个条目
		...
		$file_path = $pre_dir.($pre_dir?"/":"").$file;//构建子目录路径
		if($file=="."||$file==".."){
			continue;
		}else if(is_dir($sub_dir)&&!preg_match("/(_www|_alpha|_f5|_maker|_modules|_templates|_tests|cyber|email)$/", $sub_dir)){//如果是目录,进行递归
			echo "Directory ".$file.":<br/>";
			ForeachFiles($sub_dir, $pre_dir.($pre_dir?"/":"").$file);
		//}else if(preg_match("/_tpl\./", $file)){//电商静态页模板生成
		}else if(
		!preg_match("/(_tpl|_bak)\./", $file) &&
		//((
		!preg_match("/^design\/index\.html$/", $file_path) &&
		!preg_match("/^design\/case\/index\.html$/", $file_path) &&
		!preg_match("/^product\/.+\.html$/", $file_path) &&
		!preg_match("/^design\/category\/\d+\.html$/", $file_path) &&
		!preg_match("/^package\/index\.html$/", $file_path) &&
		!preg_match("/^(all|index)\.html$/", $file_path)
		//))
		){//生成发布页面
			echo "File in Directory: ".$file_path."<br/>";
...
function CompileJS($path, $file){
	$config = GetConfig();
	if(!preg_match("/(_bak|_json)\.js$/", $file)){
        $content = file_get_contents(WEBROOT.str_replace(DEV_ROOT,"",$path."/".$file)."?o=".TARGET);
        $msg = "文件 ".$file." 不存在".$content;
        if(!empty($content)){
		    $version = $config["version"];
