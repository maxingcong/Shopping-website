 ...
 */
require(["base", "tools", "product/detail", "product/params_com", "product/price", "product/config/com"], function ($, T, ProductDetail, ProductParamsCom, ProductPrice, CFG_PARAMS){
    var pdtDetail = ProductDetail(),
        pdtParams = ProductParamsCom(),
        ...
        pdtDetailQty = {
            ...
            init: function(){
                ...
                pdtDetail.on("change.num", function(opts){
                    //拉取价格
                    _this.getPrice();
                }).on("set.product", function(product){
                    product.hasQty = 0; //是否可自定义数量
                    product.hasCounter = 1; //支持款数
                    if(product.productId == 101 || product.productId == 102 || product.productId == 103 || product.productId == 104){
                        product.amountAlt = "（若选用第三方物流配送，请联系客服咨询运费）";
                    }
                }).on("loaded", function(data, o){debugger
                    ...
                    if(data.productId==200313) { //创意折扇特殊处理
                        options.pattr = options.pattr||"创意折扇_500_140mm*70mm";
                    }
            ...
            addCart: function(buynow){
                ...
                if(pdtParams){
					...
					var qtyValue = opts.qtyValue || (_this.options.attr["尺寸数量"].split(" ").pop());
                    if(address && productParam && opts && opts.productId && qtyValue){
                        var data = [];
                        for(var i=0; i<opts.counter; i++){
                            data.push({
                                pnum: 1,
                                quantity: qtyValue,
                                cid: opts.categoryId,
            ...
            getPrice: function(){debugger
                ...
                if(pdtParams && pdtPrice){
                    ...
                    if(address && productParam && opts && opts.productId){
                        ...
                        pdtPrice.getPrice(paramsData, opts);
                    }
                }
            }
        };
